...
const fetchBooksByFiction = async () => {
    try {
        const response = await axios.get('http://localhost:4000/api/book/fiction'); 
...
const FictionBooks  = () => {
    const { data, error, isLoading } = useQuery<bookType[]>('booksByFiction', fetchBooksByFiction);
    ...
    return (
        <div>
            <h1 className='text-sm md:text-6xl font-bold text-center mb-12 mt-12 italic' style={{ fontFamily: "Georgia, sans-serif" }}>Fiction Books</h1>
